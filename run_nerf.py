...
def train(args,images, poses, render_poses, hwf, i_train, i_val, i_test,global_step,H,W,K,basedir,expname,render_kwargs_train, render_kwargs_test, grad_vars, optimizer):
    ...
    for i in trange(start, N_iters):
        ...
        trans = extras['raw'][...,-1]
        loss = img_loss
        psnr = -10. * torch.log(img_loss) / torch.log(torch.Tensor([10.]))

        if 'rgb0' in extras:
            img_loss0 = torch.mean((extras['rgb0']-target_s)**2)
            loss = loss + img_loss0
            psnr0 = -10. * torch.log(img_loss0) / torch.log(torch.Tensor([10.]))
...
if __name__=='__main__':
    ...
    train(args,images, poses, render_poses, hwf, i_train, i_val, i_test,global_step,H,W,K,basedir,expname,render_kwargs_train, render_kwargs_test, grad_vars, optimizer)
